...
(defn key-pressed []
  ;(println (q/key-code))
  (cond
   (= 49 (q/key-code)) (q/save-frame "06-2-spirograph-####.png") ;1
  ...
  (q/with-translation [(:half-w out) (:half-h out)]
    ...
    (doseq [[index a] (map vector (iterate inc 0) (range 0 q/TWO-PI (/ q/PI (:n out))))]
      (let [skew1 (* (:grad out) a) ;; --> Meter skew en spirograph???
            ...
            alfa1 (+ skew1 a)
            alfa2 (+ skew2 a)
            x1 (* (:inner-r out)  (Math/pow (q/cos alfa1) 3)) ;; elipse: x = h + a*cos alfa1
            y1 (* (:inner-r out)  (Math/pow (q/sin alfa1) 3)) ;; elipse: y = k + b*cos alfa1
